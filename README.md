...
+ **[2025-02-04]** 完成Debug（因为大四一堆事情搁置了一个学期），支持Apifox里面的API基本运行。支持`SE-TEST.py`运行。
...
如果先启动前端，再启动的后端，那么导致前端可能会无法渲染，也就无法打开酒店管理面板。
